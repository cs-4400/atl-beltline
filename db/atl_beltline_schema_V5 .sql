...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`cs4400user`@`%` PROCEDURE `enter_emails`(IN p_emails VARCHAR(255))
BEGIN
    DECLARE big_len INT;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`cs4400user`@`%` PROCEDURE `manage_event`(IN manager VARCHAR(25))
BEGIN
    select event_name,
    description,
    event_price,
...
-- Dump completed on 2019-04-20 20:24:14
